...
export const SavedListCard: React.FC<SavedListCardProps> = ({
    ...
    return (
        <div
            className={`${colorClass} border-2 border-black dark:border-zinc-700 rounded-xl p-5 shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] hover:translate-y-[-2px] hover:shadow-[6px_6px_0px_0px_rgba(0,0,0,1)] transition-all duration-200 group relative flex flex-col h-full min-h-[240px] overflow-hidden ${rotation}`}
            style={{
        ...
        >
            ...
            <div className="flex justify-between items-start mb-4 border-b-2 border-black/10 dark:border-white/10 pb-3 mt-2">
                ...
                <div className="flex items-center gap-1">
                    <div className="flex items-center group/edit relative">
                        <span className="text-[10px] font-bold text-black bg-yellow-400 px-2 py-1 rounded border border-black transition-all duration-200 opacity-0 group-hover/edit:opacity-100 hover:scale-105 absolute -top-8 left-1/2 -translate-x-1/2 whitespace-nowrap z-30 shadow-sm cursor-pointer"
                        ...
                        <Button
                            ...
                            size="icon"
                            onClick={(e) => { e.stopPropagation(); onLoad(list); }}
                            className="h-8 w-8 text-gray-700 hover:text-black hover:bg-black/5 rounded-full"
            ...
            <div className="flex-1 overflow-hidden relative">
                <ul className="space-y-0">
                    {list.items.slice(0, 5).map((item, i) => {
                        return (
                            ...
                            >
                                ...
                                />
                                <span className={`truncate w-full font-medium ${item.checked ? 'line-through text-gray-400 decoration-2 decoration-gray-400' : ''}`}>
                                    {item.text}
                                </span>
                            </li>
                        );
                    ...
                    {list.items.length > 5 && (
                        <li className="text-xs font-bold text-gray-400 mt-2 px-2 italic">
                            {language === 'he'
            ...
            {/* Date Footer */}
            <div className="mt-auto pt-3 border-t-2 border-black/5 dark:border-white/5 flex justify-between items-center">
